...
//               addSHSection　= createSecondHierarchyHtml(shName, firstHierarchyIndex, name, secondHierarchyIndex);
               ...
               for(let index3 = 0; index3 < secondHierarchy.thirdHierarchyList.length; index3++){
                    ...
                    addTHSection =  createThirdHierarchyHtml(value, shName, thName){
...
function changeFrameWork(target){
   ...
   for (let index = 0; index < data.length; index++) {
      ...
      `</font>` + 
       ...
       $('#hierarchy').append(new_list);
         // 「第二階層を追加」ボタンを追加
         secondHierarchyButton = '<div name="addSH" id="addSh' + data[index].id + '" class="row"><button onclick="changeHierarchy(this, \'addSh\', \'fh' + index + '\')" type="button" class="btn btn-primary col-3 mb-2">第二階層を追加する</button></div>'
...
//                }
        ...
        case 'addTH':
            ...
            addHtml =  createThirdHierarchyHtml(value, shName, thName)
...
//第三階層の要素を作成します.
function createThirdHierarchyHtml(value, shName, thName){
    return addThirdHierarchyHtml = 
	 '<section name="' + thName + '">' + 
	 '<div class="row"><label for="" class="offset-1 col-2">第三階層：</label>' + 
	 '<input type="text" class="form-control col-8 float-right row3-input" value="' + value +  '">' +
	 '<button name="th" onclick="changeHierarchy(this,\'delete\',\'' + shName + '\')" type="button" class="btn btn-info col-1">削除</button>' + 
	 '</div>' + 
	 '</section>';
}
