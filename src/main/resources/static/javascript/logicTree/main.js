...
$(function(){
    ...
    $('input[name="clarify"]').change(function () {
    	$('#descriptionTypeForLogicTree').val($(this).val())
        if($( this ).val() == 1){
            // 原因(理由)を選択
            $('#reason').css("display", "block");
            $('#method').css("display", "none");
          ...
          $.ajax({
            ...
            $('#hierarchy > .fw').remove();
              ...
              for (let index = 0; index < data.length; index++) {
                  ...
                  $('#hierarchy').append(new_list);
                    // 「第二階層を追加」ボタンを追加
                    secondHierarchyButton = '<div name="addSH" id="addSh' + data[index].id + '" class="row"><button type="button" onclick="button(this, \'addSh\', \'fh' + index + '\')" class="btn btn-primary col-3 mb-2">第二階層を追加する</button></div>'
                    $('#fw' + data[index].id).append(secondHierarchyButton);
    ...
    function button(selectedId, option, name){
        ...
        switch (option) {
            ...
            case 'addFH':
                ...
                '<div class="row">' + 
                //追加部分
                '<button onclick="button(this, \'addSh\', \'' + name + '\')" type="button" class="btn btn-primary col-3 mb-2">第二階層を追加する</button>' + 
                '</div>'
